...
function RiskMeter({ score }) {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setWidth(score), 200);
      ...
      <div className="glass rounded-xl p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] text-text-secondary transition-colors hover:text-honey hover:scale-105 active:scale-95"
          >
            <Download size={11} />
            Download JSON
          </button>
...
export default function InteractiveDemo() {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          import("../data/demoData").then((mod) => setDemoCases(mod.demoCases));
          observer.disconnect();
        }
      },
      { rootMargin: "200px" }
  ...
  return (
    <section id="demo" ref={sectionRef} className="relative px-6 py-32 sm:py-40">
      ...
      <div className="mx-auto max-w-6xl">
            ...
            {(demoCases || []).map((c) => {
              ...
              return (
                <button
                  ...
                  className={`group rounded-xl border p-5 text-left transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${
                    isActive
        ...
        {activeCase && (
          <div
            key={activeCase.id}
            className="glass overflow-hidden rounded-2xl animate-slide-up"
          >
            {/* Input preview */}
              ...
              <div className="flex items-center justify-center py-24 animate-fade-in">
                <div className="flex flex-col items-center gap-5">
                  ...
                  <div className="relative h-px w-48 overflow-hidden rounded-full bg-white/[0.04]">
                    <div className="absolute inset-0 h-full w-1/3 rounded-full bg-gradient-to-r from-transparent via-honey to-transparent animate-scan-bar" />
                  </div>
                </div>
              </div>
            )}
            ...
            {showResult && (
              <div className="p-6 animate-fade-in">
                  ...
                  <div className="space-y-2">
                    {activeCase.result.signals.map((s, i) => (
                      <div
                        ...
                        style={{ animation: `slideInLeft 0.4s ease ${0.3 + i * 0.08}s both` }}
                      >
                ...
                <div
                  ...
                  style={{ animation: "slideUp 0.4s ease 0.6s both" }}
                >
                ...
                <div
                  className="mb-8"
                  style={{ animation: "slideUp 0.4s ease 0.7s both" }}
                >
                  <h4 className="mb-4 text-[10px] font-semibold tracking-widest text-text-muted uppercase">
                    What to do next
                  </h4>
                  <ol className="space-y-2.5">
                    {activeCase.result.nextSteps.map((step, i) => (
                      <li
                        ...
                        style={{ animation: `slideInLeft 0.4s ease ${0.75 + i * 0.06}s both` }}
                      >
                ...
                {/* Receipt toggle */}
                <div style={{ animation: "fadeIn 0.4s ease 0.9s both" }}>
                  <button
                    onClick={() => setShowReceipt(!showReceipt)}
                    className="glass flex items-center gap-2 rounded-full px-5 py-2.5 text-[12px] font-medium text-text-secondary transition-all hover:text-cream hover:scale-[1.03] active:scale-[0.97]"
                  >
