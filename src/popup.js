...
function formatFamilyReceiptText(fr) {
  const lines = [
    ...
    `URL: ${fr.url}`,
    `Threat level: ${(fr.threatLevel || 'unknown').toUpperCase()}`,
    '',
    `Summary: ${fr.summary}`,
    '',
    'SIGNALS DETECTED:'
...
function renderFamilyPanel() {
  ...
  if (!familyReceipts || familyReceipts.length === 0) {
    container.innerHTML = `
      <div class="family-empty">
        <div class="family-icon">\ud83d\udc68\u200d\ud83d\udc69\u200d\ud83d\udc67</div>
        <p>No family rescue receipts yet.<br><span style="color:#444;font-size:11px">When STING blocks a scam, click \u201cSend to Family\u201d to save a receipt here.</span></p>
      </div>`;
    return;
  }

  const cards = familyReceipts.map((fr, index) => {
    const threatClass = getThreatClass(fr.threatLevel);
    return `<div class="family-card" data-family-index="${index}">
      <div class="family-meta">
        <span class="family-date">${escapeHtml(formatFamilyDate(fr.timestamp))}</span>
        <span class="family-threat ${threatClass}">${escapeHtml(fr.threatLevel || 'unknown')}</span>
      </div>
      <div class="family-site">${escapeHtml(fr.hostname || fr.url || 'Unknown site')}</div>
      <div class="family-summary">${escapeHtml(fr.summary || '')}</div>
      <button class="family-copy-btn" data-family-copy="${index}">Copy receipt</button>
    </div>`;
  }).join('');

  container.innerHTML = `
    <div class="family-header">
      <h4>Family Rescue Receipts</h4>
      <button class="family-clear-btn" id="family-clear-all">Clear all</button>
    </div>
    ${cards}`;

  container.querySelectorAll('[data-family-copy]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const idx = parseInt(btn.dataset.familyCopy, 10);
      const fr = familyReceipts[idx];
      if (!fr) return;
      await navigator.clipboard?.writeText(formatFamilyReceiptText(fr));
      btn.textContent = '\u2705 Copied!';
      setTimeout(() => { btn.textContent = 'Copy receipt'; }, 3000);
    });
  });

  const clearBtn = document.getElementById('family-clear-all');
  if (clearBtn) {
    clearBtn.addEventListener('click', async () => {
      familyReceipts = [];
      if (globalThis.chrome?.storage?.local) {
        chrome.storage.local.set({ [FAMILY_RECEIPTS_KEY]: [] });
      } else {
        localStorage.setItem(FAMILY_RECEIPTS_KEY, '[]');
      }
      renderFamilyPanel();
    });
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const data = await getStorage([STORAGE_KEY, HISTORY_KEY, FAMILY_RECEIPTS_KEY]);
  const receipt = data[STORAGE_KEY] || null;
  scanHistory = data[HISTORY_KEY] || [];
  familyReceipts = data[FAMILY_RECEIPTS_KEY] || [];
  latestReceipt = receipt;
